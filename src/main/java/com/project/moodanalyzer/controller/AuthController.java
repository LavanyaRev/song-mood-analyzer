...
public class AuthController {
    ...
    public ResponseEntity<?> signup(@RequestBody User user) {
        ...
        if (existing.isPresent()) return ResponseEntity.badRequest().body("Email already registered");
    ...
    public ResponseEntity<?> login(@RequestBody User user) {
        Optional<User> existing = authService.findByEmail(user.getEmail());
        if (existing.isEmpty() || !passwordEncoder.matches(user.getPassword(), existing.get().getPassword()))
